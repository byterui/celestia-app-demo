...
func (suite *IntegrationTestSuite) TestMintDenomMsg() {
	// Create a denom
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			amount:    10,
			mintDenom: "factory/demo1wxhrdslu9w2phqqx6yxqz4vk5nfejm7lf7fsr0/evmos",
		...
		suite.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			app.AssertEventEmitted(ctx, types.TypeMsgMint, tc.expectedMessageEvents)
		})
	}
}

// TestBurnDenomMsg tests TypeMsgBurn message is emitted on a successful burn
func (suite *IntegrationTestSuite) TestBurnDenomMsg() {
	// Create a denom.
	suite.CreateDefaultDenom()
	...
	for _, tc := range []struct {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			// Ensure current number and type of event is emitted
			app.AssertEventEmitted(ctx, types.TypeMsgBurn, tc.expectedMessageEvents)
		})
	}
}

// TestCreateDenomMsg tests TypeMsgCreateDenom message is emitted on a successful denom creation
func (suite *IntegrationTestSuite) TestCreateDenomMsg() {
	defaultDenomCreationFee := types.Params{DenomCreationFee: sdk.NewCoins(sdk.NewCoin("demo", sdk.NewInt(50000000)))}
	for _, tc := range []struct {
		desc                  string
		...
		suite.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			// Ensure current number and type of event is emitted
			app.AssertEventEmitted(ctx, types.TypeMsgCreateDenom, tc.expectedMessageEvents)
		})
	}
}

// TestChangeAdminDenomMsg tests TypeMsgChangeAdmin message is emitted on a successful admin change
func (suite *IntegrationTestSuite) TestChangeAdminDenomMsg() {
	for _, tc := range []struct {
		desc                    string
		...
		suite.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			// Ensure current number and type of event is emitted
			app.AssertEventEmitted(ctx, types.TypeMsgChangeAdmin, tc.expectedMessageEvents)
		})
	}
}

// TestSetDenomMetaDataMsg tests TypeMsgSetDenomMetadata message is emitted on a successful denom metadata change
func (suite *IntegrationTestSuite) TestSetDenomMetaDataMsg() {
	// setup test
	suite.SetupTest()
	suite.CreateDefaultDenom()
	...
	for _, tc := range []struct {
		...
		suite.Run(fmt.Sprintf("Case %s", tc.desc), func() {
			...
			// Ensure current number and type of event is emitted
			app.AssertEventEmitted(ctx, types.TypeMsgSetDenomMetadata, tc.expectedMessageEvents)
		})
	}
}
